...
이 앱은 메뉴바/트레이에 상주하면서:
- 세션·주간 사용량을 **컬러 프로그레스 바**로 보여주고
- 한도에 가까워지면 **알림**을 보내고
- 내 사용 패턴에 맞는 **플랜까지 추천**해줍니다.
- 사용량에 따라 색과 속도가 변하는 **로봇 애니메이션** 🤖
...
- 세션(5h), 주간(전 모델), 주간(Sonnet) 컬러 프로그레스 바
- 사용량에 따라 🟢 초록 → 🟡 노랑 → 🔴 빨강 색상 변화
- 리셋까지 **남은 시간** 표시 (예: `↻ 2시간 58분 남음`)
- 팝오버 상단에 **Claude · Codex 사용량을 한 줄 압축 행으로 동시 표시**

### 메뉴바 타이틀 커스터마이즈
메뉴바에 보여줄 지표를 **최대 2개 슬롯**까지 자유롭게 고를 수 있습니다 (설정 → 타이틀 슬롯 1/2).
- Claude 5h·주간, Codex 5h·주간 중 원하는 조합 (예: `5h 95% | 5h 4%`)
- **색상으로 프로바이더 구분** — Claude 주황 / Codex 파랑, 앞의 `5h`/`7d` 태그로 윈도우 구분

### Codex 사용량 (로컬)
Codex CLI 로컬 로그를 읽어 5시간·주간 사용률과 토큰을 함께 표시합니다.

### 로봇 애니메이션 🤖
RunCat 스타일로, 메뉴바의 작은 로봇 아이콘이 안테나·눈을 깜빡이며 살아 움직입니다.
- **사용량이 높을수록 빠르게 움직입니다** (0% → 느긋, 높을수록 분주)
- **사용량에 따라 로봇 색이 변합니다** (흰색 → 🟡 노랑 → 🔴 빨강)
- 라이트/다크 모드 자동 대응
- 설정에서 on/off 가능

### 세부 사용량
오늘, 세션(5h), 주간(7d) 단위로 토큰, 비용, 요청 수를 깔끔하게 정렬하여 보여줍니다.

<p align="center">
  <img src="screenshots/detail-usage-v3.png" alt="세부 사용량" width="400">
</p>

### 모델별 사용량
어떤 모델에 비용을 얼마나 쓰고 있는지 색상별 프로그레스 바와 비중(%)으로 한눈에 확인할 수 있습니다.

<p align="center">
  <img src="screenshots/model-breakdown-v4.png" alt="모델별 사용량" width="400">
</p>

### 7일 사용량 히스토리
컬러 바 차트로 일별 사용 패턴을 한눈에 파악하세요. 막대에 마우스를 올리면 금액이 표시됩니다.

<p align="center">
  <img src="screenshots/7day-history-v4.png" alt="7일 히스토리" width="400">
</p>

### 플랜 추천
30일 사용량을 분석해서 Pro / Max 5x / Max 20x 중 나에게 맞는 플랜을 추천합니다.
플랜별 한도 사용률을 프로그레스 바로 직관적으로 비교할 수 있습니다.

<p align="center">
  <img src="screenshots/plan-recommendation-v4.png" alt="플랜 추천" width="400">
</p>

### 기타
- **사용량 임계치 알림** — 80%, 90% 도달 시 시스템 알림
